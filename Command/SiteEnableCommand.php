...
class SiteEnableCommand extends ContainerAwareCommand
{
    /**
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('name', null, InputOption::VALUE_REQUIRED, 'Which site do you want to enable?', null)
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if(is_null($name)) {
            $output->writeln('<error>Please enter the name of site you want to enable. Ex: site:enable --name=demo</error>');
            return;
        }
        ...
        if(isset($sites[$name])) {
            ...
            if(count($sites[$name]) > 1) {
                ...
                $question = new Question('Please enter the host you want to enable:', null);
            ...
            foreach ($sites[$name] as $key => $site) {
                if($site['host'] == $host) {
                    ...
                    $question = new ConfirmationQuestion('Do you want to enable ? (yes/no) ', false);
                    if($helper->ask($input, $output, $question)) {
                        $enabled = true;
                        $sm->updateSiteConfigs($sites);
                        $output->writeln('');
                        $output->writeln('<fg=green>Success: The host '.$host.' has been enabled</>');
                        $output->writeln('');
                    }
                }
            }
            ...
            if(!$enabled) {
                $output->writeln('');
                $output->writeln('<fg=red>The host '.$host.' has not been enabled</>');
                $output->writeln('');
            }
        } else {
